...
parser.add_argument('--model', '-m', type=str, default='mlp', # default allconv
                    choices=['allconv', 'wrn', 'densenet', 'mlp','wrn_ssnd'], help='Choose architecture.')
...
if args.load_pretrained == 'snapshots/pretrained':
    ...
    for i in range(200, -1, -1):
        ...
        if os.path.isfile(model_name):
            ...
            if args.model in ['wrn_ssnd']:
                wrn_ssnd = WideResNet_SSND(net)
                net.load_state_dict(wrn_ssnd.wrn.state_dict())
...
if args.score in ['woods_nn']:
    ...
    # freeze wrn layers
    for layer in net.wrn.parameters():
...
def train(epoch):
    ...
    for in_set, aux_in_set, aux_in_cor_set, aux_out_set in loaders:    
        ...
        if args.score == 'woods_nn':
            ...
            #flex_eta = 2/torch.norm(list(net.parameters())[-2]).item()
            #out_loss = torch.mean(F.relu(flex_eta - out_x_ood_task))
            out_loss = torch.mean(F.relu(1 - out_x_ood_task))
            out_loss_weighted = args.out_constraint_weight * out_loss
            delta_eta.append(torch.norm(list(net.parameters())[-6]).item())
            ...
            #idx = int(len(in_set[0])+args.batch_size*(1-args.pi_1-args.pi_2))
            energy_out = -torch.logsumexp(x[len(in_set[0]):], dim=1).detach()
            energy_in = -torch.logsumexp(x[:len(in_set[0])], dim=1).detach()
            ...
            #update eta
            Eod.append(torch.mean(energy_out).cpu())
            if Eod[-1] <= args.eta:
                #delta_eta.append(torch.div(torch.sum(torch.exp(energy_in))*(1-args.pi_1-args.pi_2),torch.sum(torch.exp(energy_out))-torch.sum(torch.exp(energy_in))*(1-args.pi_1-args.pi_2)).item())
                delta_eta.append(torch.div(torch.sum(torch.exp(energy_in)),torch.sum(torch.exp(energy_out))-torch.sum(torch.exp(energy_in))).item())
            else:
            ...
            if ce_constraint_weight * loss_ce_constraint + lam2 >= 0:
                loss_ce = loss_ce_constraint * lam2 + ce_constraint_weight / 2 * torch.pow(loss_ce_constraint, 2)
                #args.eta = args.eta - torch.div(torch.sum(torch.exp(energy_in)),torch.sum(torch.exp(energy_out)))
            else:
                loss_ce = - torch.pow(lam2, 2) * 0.5 / ce_constraint_weight
    ...
    if epoch ==0:
        args.eta = np.mean(Eod)
    delta = np.mean(delta_eta)
    #if delta <= args.false_alarm_cutoff:
    #    delta = 0
    args.eta = args.eta - delta
    state['etas'].append(args.eta)
    

    #if args.score in ['woods_nn']:
    #    print('Update margin:',flex_eta)
...
for epoch in range(0, args.epochs):
    ...
    if args.score in ['scone']:
        print('Update margin:',args.eta)
    ...
    #if epoch%10 == 0:
    #    args.checkpoints_dir = './snapshots/save/'
